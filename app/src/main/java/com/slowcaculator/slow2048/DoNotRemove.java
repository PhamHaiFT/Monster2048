...
public class DoNotRemove {
    ...
    private static DoNotRemove doNotRemove;

    public static DoNotRemove getInstance() {
        if (doNotRemove == null) {
             doNotRemove = new DoNotRemove();
        } return doNotRemove;
    }
    ...
    private final ArrayList<Integer> listNumber = new ArrayList<>();
    private int[] listColor;
    private final int[][] twoDimensionArray = new int[4][4];
    ...
    public void initSaved(Context context,ArrayList<Integer> dataSave) {
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
                twoDimensionArray[i][j] = dataSave.get(loopCount);
                loopCount++;
//                listNumber.add(twoDimensionArray[i][j]);
            }
        }

        //List color for block
        TypedArray typedArray = context.getResources().obtainTypedArray(R.array.listColor);
        listColor = new int[typedArray.length()];
        for (int i = 0; i < listColor.length; i++) {
            listColor[i] = typedArray.getColor(i, 0);
